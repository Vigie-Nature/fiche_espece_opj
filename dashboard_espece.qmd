...
gg = df_repartition %>%
  ggplot() +
    ...
    ylab("% d'abondance") +
    ggtitle("Abondance de chaque espèce parmi toutes les observations") +
    theme_cowplot() +
...
carte_ab(shape_map = france, fill_map = df_dep$cl_ab, fill_color = couleurs,
         ...
         map_title = paste0("Carte de distribution de l'abondance totale de l'espèce ", sp_name))
```
...
# Histo test {orientation="columns"}
...
ggplot(df_histo_test_3, aes(x = reorder(nom_espece, classif), fill = ab_grega)) +
  geom_bar(aes(weight = prop_grega)) +
