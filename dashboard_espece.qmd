...
format:
  dashboard:
    ...
    css: ../programs/styles_dashboard.css
execute:
  echo: false
  message : false
  warning : false
...
```{r}
#| title: "Abondance de l'espèce et observations totales"
...
for(an in sort(unique(df_all_sp$annee))) {
  ...
  print(carte_ab(shape_map = france, fill_map = df_dep_an$cl_moy,
                 ...
                 fill_title = paste0("Abondance relative en ", an)))

}
```
:::

Pour une année, on calcule dans chaque département le nombre d'individus vus en moyenne dans un jardin.
...
Grégarité moyenne : on calcule la moyenne de l'abondance et l'écart-type pour toutes les observations d'une espèce où au moins un individu a été observé.
...
`{r} paste0(nb_jardin_obs, " sur ", length(unique(df_all_sp$jardin_id)), " jardins")`
:::
