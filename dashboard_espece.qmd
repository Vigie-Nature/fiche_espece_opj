...
`{r} paste0(label_number(accuracy = 10)(nb_obs_idv), " sur ", label_number(accuracy = 10)(nrow(df_sp)), " au total")`
:::

::: {.valuebox icon="eye-fill" color="#5bbdd6"}
Nombre total d'individus observés :

`{r} paste0(label_number(accuracy = 10)(nb_idv_cpt), " sur ", label_number(accuracy = 10)(sum(df_all_sp$abondance)), " au total")`
:::
...
ggplot(df_date_sqrt, aes(x = annee, y = sum_sp)) +
...
for(i in 1:3) {
  ...
  print(df_obs %>%
    ggplot() +
      ...
      xlab(lst_param[[i]][[3]]) +
      ylab(paste0("% des observations dans une catégorie\n(pour l'espèce et au total)")) +
      theme_cowplot())
