...
Les participants ont compté `r paste0(label_number(accuracy = 10)(nb_idv_cpt), " ", sp_name, " sur les ", label_number(accuracy = 10)(sum(df_opj$abondance)), " papillons au total.")`
:::
...
### 2006-2018
```{r}
gg_histo_plotly(df_hp = df_repartition_old, limits = df_repartition_old$nom_espece,
...
### 2006-2024
```{r}
gg_histo_plotly(df_hp = df_repartition_new_old, limits = df_repartition_new_old$nom_espece,
...
datemax = max(df_sp$date) + 7

gg1 <- gg_histo(df_histo = df_ab_rel %>% filter(annee >= 2019),
...
gg1 <- gg_histo(df_histo = df_ab_rel,
                y = "sum_ab_rel", dmin = datemin, dmax = datemax,
                ytxt = "Abondance relative", title = "Abondance relative par semaine")
...
#### 2019-2024
```{r}
vec_col = c(colfunc(length(unique(df_sp_new$annee))-1), "#f40b0b")
aes_echarts(plot_e = df_freq_rel_new %>%
              group_by(annee) %>%
...
#### 2006-2018
```{r}
vec_col = c(colfunc(length(unique(df_sp_old$annee))-1), "#f40b0b")
aes_echarts(plot_e = df_freq_rel_old %>%
              group_by(annee) %>%
              e_charts(num_semaine) %>%
              e_line(freq_rel, symbol='none'),
            xlab = "Semaine de participation",
