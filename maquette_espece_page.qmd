...
- Suppression des départements en NA
- Suppression des départements avec plus de 2 caractères (suppression des drom-com)
- Suppression des dates antérieures au 1 janvier 2005 si données historiques, sinon 1 janvier 2019
- Suppression des observations ne concernant pas des taxons de la liste principale
(Machaons,Flambés, Demi-deuils,
...
- un data frame de toutes les observations (df_opj)
- un data frame avec uniquement le taxon de la fiche (df_sp)
...
- Filtre sur les données où l'abondance est strictement supérieure à 0 sur df_sp puis comptage du nombre de lignes
...
- Filtre sur les données où l'abondance est strictement supérieure à 0 sur df_sp puis somme de toutes les abondances
...
- On a besoin des noms des taxons et des abondances correspondantes.
- On groupe par taxon et on somme les abondances de chaque taxon. On divise ensuite les abondances de chaque taxon par l'abondance totale de tous les taxons.
...
En lisant la figure on peut observer que le classement de l'espèce parmi les 28 espèces de la liste principale : `r paste0(which(rev(df_repartition$taxon) == sp_name), "/", length(df_repartition$taxon))`
...
- On remplace par les raster
- Depuis le df_sp, on a besoin des jardins, de leur latitude, longitude et de l'abondance du taxon
- On groupe par jardin et on somme l'abondance. On divise ensuite en deux catégories : observé (> 0) et non observé (= 0)
...
Nouveau type de représentation de la cartographie
...
- Depuis df_sp, on groupe par date. On somme l'abondance, le nombre d'observations (toutes les observations où l'abondance du taxon est supérieure à 0), et le nombre de sessions uniques. On calcule ensuite l'abondance relative en divisant la somme d'abondance par le nombre de sessions.
...
- On considère le nombre d'individus vus en même temps comme une catégorie, et on compte le nombre de sessions rattachées à chaque catégorie. Ensuite on divise en trois catégories (1 / 2 à 4 / 5 et +) et dans le graphique on fait un histogramme pour chaque catégorie.
